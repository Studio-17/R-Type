                    ...
                    if (types[x] && positions[x] && rects[x]) {
                        ...
                        if (CheckCollision(firstrect, secondrect, firstpos, secondpos)) {
                            if (secondtype.value().type == PLAYER || secondtype.value().type == BULLET) {
                                network_queues[0].value().toSendNetworkQueue.push(serialize_header::serializeHeader<packet_kill_entity>(NETWORK_SERVER_TO_CLIENT::KILL_ENTITY, {registry.entity_from_index(i)}));
                                network_queues[0].value().toSendNetworkQueue.push(serialize_header::serializeHeader<packet_kill_entity>(NETWORK_SERVER_TO_CLIENT::KILL_ENTITY, {registry.entity_from_index(x)}));
