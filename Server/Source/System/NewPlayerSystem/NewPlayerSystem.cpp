...
void NewPlayerSystem::operator()(Registry &registry, Sparse_array<component::cnetwork_queue_t> &netqueue, Sparse_array<component::cposition_t> &position, Sparse_array<component::ctype_t> &type)
{
    while (!netqueue[0].value().newPlayerQueue.empty()) {
        ...
        if (position[spaceShip] && type[spaceShip]) {
            ...
            netqueue[0].value().toSendNetworkQueue.push(serialize_header::serializeHeader<packet_new_entity>(NETWORK_SERVER_TO_CLIENT::PACKET_TYPE::NEW_PLAYER, {static_cast<uint16_t>(spaceShip), position[spaceShip].value().x, position[spaceShip].value().y, 1, static_cast<size_t>(type[spaceShip].value().type)}));
            for (std::size_t index = 1; index != position.size(); index++) {
                if (position[index] && type[index]) {
                    if (index == spaceShip)
                        continue;
                    netqueue[0].value().toSendNetworkQueue.push(serialize_header::serializeHeader<packet_new_entity>(NETWORK_SERVER_TO_CLIENT::PACKET_TYPE::NEW_ENTITY, {static_cast<uint16_t>(registry.entity_from_index(index)), position[index].value().x, position[index].value().y, 1, static_cast<size_t>(type[index].value().type)}));
                }
            }
        }
